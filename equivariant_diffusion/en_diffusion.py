...
import sys
sys.path.append('/data/yfang/EDM_new/')
...
class EnVariationalDiffusion(torch.nn.Module):
    ...
    def sample_p_zs_given_zt(self, s, t, zt, node_mask, edge_mask, context, fix_noise=False,cond_fn=None, guidance_kwargs=None,prior_network=None,scale=5.0,is_visual=False):
        ...
        if cond_fn is not None and guidance_kwargs is not None:
            if is_visual:
                mu,grad = self.condition_mean(cond_fn, mu, sigma**2, zt, t,node_mask, guidance_kwargs,prior_network,scale,is_visual)
            else:
                mu = self.condition_mean(cond_fn, mu, sigma**2, zt, t,node_mask, guidance_kwargs,prior_network,scale,is_visual)
    ...
    @torch.no_grad()
    def sample(self, n_samples, n_nodes, node_mask, edge_mask, context, fix_noise=False,cond_fn=None, guidance_kwargs=None,prior_network=None,scale=5.0):
        """
        ...
        for s in reversed(range(0, self.T)):
            ...
            z = self.sample_p_zs_given_zt(s_array, t_array, z, node_mask, edge_mask, context, fix_noise=fix_noise,cond_fn=cond_fn, guidance_kwargs=guidance_kwargs,prior_network=prior_network,scale=scale,is_visual=False)
    ...
    @torch.no_grad()
    def sample_ob(self, n_samples, n_nodes, node_mask, edge_mask, context, fix_noise=False,cond_fn=None, guidance_kwargs=None,prior_network=None,scale=5.0):
        """
        Draw samples from the generative model.
        """
        if fix_noise:
        ...
        for s in reversed(range(0, self.T)):
            ...
            z = self.sample_p_zs_given_zt(s_array, t_array, z, node_mask, edge_mask, context, fix_noise=fix_noise,cond_fn=cond_fn, guidance_kwargs=guidance_kwargs,prior_network=prior_network,scale=scale)
        ...
        if max_cog > 5e-2:
            print(f'Warning cog drift with error {max_cog:.3f}. Projecting '
                  f'the positions down.')
            x = diffusion_utils.remove_mean_with_mask(x, node_mask)
            x_clean = diffusion_utils.remove_mean_with_mask(x_clean, node_mask)
    ...
    def sample_ob_sim(self, n_samples, n_nodes, node_mask, edge_mask, context, fix_noise=False,cond_fn=None, guidance_kwargs=None,prior_network=None,scale=5.0,epoch=20,start_iter=1000,threshold=0.01):
        ...
        def cal_sim(z,guidance_kwargs,t):
            if t<50: #lcoal model
                model = guidance_kwargs[3]
            else:
                model = guidance_kwargs[2]
            ##text embedding
            text = guidance_kwargs[0]
            text_mask = guidance_kwargs[1]

            text_features = model.encode_text(text,text_mask).cpu()
            emb_text = np.array(text_features)

            ##3D molecular embedding
            graph = data2fc_graph(z,node_mask,[0])[0]
            graph = graph.to(device)

            
            #graph.batch
            batch_ = torch.zeros((181),dtype=torch.int64).to(device)

            xxx = torch.zeros(1)
            timestep = torch.tensor([t])
            timestep = timestep.to(device)
            mol_features, xxx = model(graph.x, graph.edge_index, batch_, xxx, timestep, node_mask)
            mol_features = mol_features.cpu()
            emb_3D = np.array(mol_features)

            flat_vector1 = emb_text.flatten()
            flat_vector2 = emb_3D.flatten()

            dot_product = np.dot(flat_vector1, flat_vector2)
            norm1 = np.linalg.norm(flat_vector1)
            norm2 = np.linalg.norm(flat_vector2)
        ...
        for s in reversed(range(0, self.T)):
            ...
            z_clean =  self.sample_p_zs_given_zt(s_array, t_array, z_clean, node_mask, edge_mask, context, fix_noise=fix_noise,cond_fn=None, guidance_kwargs=None,prior_network=None,scale=scale)
            sim_z_clean = cal_sim(z_clean,guidance_kwargs,s)

            if s < start_iter:
                for curr_epoch in range(epoch):
                    if s<50: #local_model
                        z,grad = self.sample_p_zs_given_zt(s_array, t_array, z, node_mask, edge_mask, context, fix_noise=fix_noise,cond_fn=cond_fn, guidance_kwargs=[guidance_kwargs[0],guidance_kwargs[1],guidance_kwargs[3]],prior_network=prior_network,scale=scale,is_visual=True)
                    else: #global model
                        z,grad = self.sample_p_zs_given_zt(s_array, t_array, z, node_mask, edge_mask, context, fix_noise=fix_noise,cond_fn=cond_fn, guidance_kwargs=guidance_kwargs[:-1],prior_network=prior_network,scale=scale,is_visual=True)
                    sim_z = cal_sim(z,guidance_kwargs,s)
                    #sim.append(('timestep{}'.format(s) , 'epoch {}'.format(curr_epoch),sim_z,sim_z_clean))
                    grad_norm = torch.norm(grad)
                    sim.append((s,curr_epoch,sim_z,sim_z_clean,grad_norm.item()))

                    #threshold clip
                    if sim_z_clean - sim_z > threshold:
                        z = z_clean
                ...
                z = self.sample_p_zs_given_zt(s_array, t_array, z, node_mask, edge_mask, context, fix_noise=fix_noise,cond_fn=None, guidance_kwargs=None,prior_network=None,scale=scale)
        ...
        return x, h, x_clean, h_clean,sim     
    ...
    @torch.no_grad()
    def q_sample(self, x, h, t_int, node_mask):
        """Computes z_t given x_h and t"""
        s_int = t_int - 1
        # Normalize t to [0, 1]. Note that the negative
        # step of s will never be used, since then p(x | z0) is computed.
        s = s_int / self.T
    ...
    def data2fc_graph(self,mol_data,node_mask):
        graph_list=[]
        for mol_idx in range(mol_data.size(0)):
            ...
            graph = torch_geometric.data.Data(x=mol_feature, edge_index=edge_index)
            graph_list.append(graph)
        return graph_list

    def condition_mean(self, cond_fn, mean,variance, x,t, node_mask,guidance_kwargs=None,prior_network=None,scale=5.0,is_visual=False):
        """
        ...
        """
        mol_graph = self.data2fc_graph(x,node_mask)
